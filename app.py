...
def graficos():
    ...
    data_curtidas_idades = [x[1].strftime('%Y-%m-%d') for x in mediaidades_curtidas]
    idpublicacoes = [x[0] for x in mediaidades_curtidas]

    data_comentarios_idades = [x[1].strftime('%Y-%m-%d') for x in mediaidades_comentarios]
    ...
    dict_merge = {'curtidas': dict_curtidas, 'comentarios': dict_comentarios, 'media': media}
